...
async function runReplaceDefaults(templateCwd2) {
  ...
  await run(path3.join(__dirname2, "codemods", "replaceDefaults.js"), files, { parser: "tsx", dry: false });
  try {
...
async function setupComponentFoldersAndRoutes(templateCwd2) {
  ...
  for (const project of plasmicData.projects) {
    for (const component of project.components) {
      const srcDirPath = path3.resolve(templateCwd2, plasmicData.srcDir);
      const componentFile = path3.resolve(srcDirPath, `${component.name}.tsx`);
      const componentDirPath = path3.resolve(srcDirPath, component.name || "");
      if (!await fs2.pathExists(componentDirPath)) {
  ...
  await run(
    path3.join(__dirname2, "codemods", "addRoutes.js"),
    [appTsxPath],
...
async function runUpdateImportPathsCodemod(file) {
  await run(
    path3.join(__dirname2, "codemods", "updatePlasmicImportPath.js"),
    [file],
