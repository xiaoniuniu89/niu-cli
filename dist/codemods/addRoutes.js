    ...
    root.find(j.JSXElement, { openingElement: { name: { name: 'Route' } } }).forEach(path => {
        ...
        path.node.openingElement.attributes.forEach(attr => {
            ...
            if (attr.name && attr.name.name === 'path') {
                // @ts-ignore
                existingRoutes.add(attr.value.value); // 'attr.value' can be JSXExpression or StringLiteral
            }
        });
    });
        ...
        j(path).find(j.JSXElement, { openingElement: { name: { name: 'Router' } } })
            ...
            pagesComponents.forEach(component => {
                ...
                if (!existingRoutes.has(routePath)) {
                    const newRoute = j.jsxElement(j.jsxOpeningElement(j.jsxIdentifier('Route'), [
                        j.jsxAttribute(j.jsxIdentifier('path'), j.literal(routePath)),
                        j.jsxAttribute(j.jsxIdentifier('element'), j.jsxExpressionContainer(j.jsxElement(j.jsxOpeningElement(j.jsxIdentifier(component.name), [], true), null, [])))
