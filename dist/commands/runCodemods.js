...
export async function runReplaceDefaults(templateCwd) {
...
export async function setupComponentFoldersAndRoutes(templateCwd) {
    ...
    for (const project of plasmicData.projects) {
        for (const component of project.components) {
            ...
            const componentDirPath = path.resolve(srcDirConcat, component.name);
            if (!(await fs.pathExists(path.resolve(srcDirConcat, component.name)))) {
...
export async function runUpdateImportPathsCodemod(file) {
    const jscodeshiftOptions = {
        parser: 'tsx',
        dry: false,
    };
